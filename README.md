...
**Status (2026-08-26): two-camera stereo version confirmed working end-to-end on real hardware — detection accuracy and inference speed both fixed this session; one false-positive bug still open.** Goal: robot patrols a known, bounded room's perimeter, detects a duck somewhere near the center, and reports its estimated grid-cell location — a scoped-down "search and rescue" framing (patrol + detect + report), pitched by teammate Alex as a more concrete, demoable narrative than the original generic semantic-navigation goal.
...
- **No Isaac ROS Docker / dense stereo point cloud** — still true, but the distance method itself changed 2026-08-25 (see the stereo upgrade section below). Original single-camera height-based formula (`distance = known_height * focal_length / bbox_height_px`, needed the duck's real height hardcoded) replaced with the robot's own lightweight two-camera stereo: run detection on both cameras, use the pixel disparity between them + the known ~8.3cm camera separation to triangulate distance directly — no object-size assumption, no Docker. Old approach kept as `search_and_rescue_monocular_backup.py`.
...
| 6 🔄 | Semantic navigation — scoped to search-and-rescue: perimeter patrol + two-camera stereo duck detection + live dashboard, see [Search & Rescue Mission MVP](#search--rescue-mission-mvp-jetsonmission) above; vision pipeline confirmed on real hardware (GPU inference, `mAP50=0.984`), one false-positive bug still open, full patrol+motors test pending (Alex) |
