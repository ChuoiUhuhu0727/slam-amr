...
- **Open, unresolved: `vo_pose` position values are not physically plausible.** Fast hand-movement of the camera rig caused values to diverge into the thousands within seconds (confirmed this was fast-motion-induced tracking loss, not a pipeline bug, by cross-checking with vịt: movement was fast, scene had normal texture — classic motion-blur/large-inter-frame-displacement VSLAM failure, not a config error). A clean-restart + deliberately slow motion test did **not** explode, but settled into an oscillating band (~x=11-16, not the small centimeter-scale values seen in the very first post-launch reading of 0.04/-0.07/-0.03) — looks like a **scale error** rather than lost tracking. **Next session, start here:** investigate whether `rectified_images:=False` mode is correctly interpreting the calibration's units/distortion coefficients — a good comparison test is running the same calibration through a proper `rectified_images:=True` path (with an explicit rectification node) and seeing whether the scale comes out correct there, which would localize the bug to the raw-mode camera_info interpretation specifically.
