...
**OPEN as of session end, resume here:**
1. **Point cloud not visible in RViz2 despite the topic/TF being correctly wired.** Last live check (before the Jetson lost network entirely) showed only the sphere/text markers rendering, no visible `PointCloud2` points. Not yet isolated whether this is an RViz2 display-config issue (default point `Size` is easy to leave too small to see) or the backend genuinely not publishing at that moment — check `ros2 topic hz /duck_pointcloud_region` directly before assuming either.
2. **Real ruler ground-truth benchmark for the point-cloud method — still not done.** Live agreement with the centroid method (within 1-2cm) is a promising sign but not proof of accuracy; both methods could share a common bias. Same protocol as the 0.5/1.0/1.5m centroid-only test above, this time logging both methods side by side (already easy — both show on the same `/vision` page).
3. **Jetson lost network connectivity entirely at session end** (both direct IP and mDNS unreachable, not just slow) — cause unconfirmed, needs a physical check (WiFi state, whether it's hung) before resuming.
