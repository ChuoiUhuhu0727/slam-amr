...
![](<../docs/images/H-Bridge in TB6612FNG.drawio.png>)
...
### Stereo Distance Accuracy + Dashboard Display Bug (2026-08-27) — RESOLVED

**Ruler benchmark done: 0.5m→0.46m (8.0% err), 1.0m→1.06m (6.0% err), 1.5m→1.35m (10.0% err, raw single-frame reading fluctuated 1.2–1.7m at that distance).** Root cause of the fluctuation: `distance = fx_rect * baseline_m / disparity_px` is a 1/x relationship — at range, disparity is already small, so the same few-px bounding-box jitter swings computed distance much harder than it does up close (same formula, opposite-end failure mode to the already-known <13cm close-range breakdown).

**Fix #1 (merged): rolling-median smoothing.** `STEREO_SMOOTHING_WINDOW=8`-frame rolling median of `distance_m`, shown alongside the raw reading on the dashboard ("X.XXm raw / Y.YYm smoothed"). Window sized for the ~5Hz detect loop (~1.6s lag) — deliberately smaller than the 10-20 frames a commercial stereo camera might use internally, since this loop runs much slower than that class of hardware.

**Fix #2 (merged): duck size-sanity gate.** Rejects a stereo sighting when the implied real-world height (box height × distance ÷ focal length, pinhole projection) is way off `DUCK_HEIGHT_M=0.13m` — catches the class of bad reading seen live (e.g. 0.13-0.15m distance from 500-590px disparity, right in the known-unreliable close-range zone). Chosen over retraining the model specifically to avoid a whack-a-mole regression risk (fixing one false-positive case while silently breaking another, with no fixed regression-test set) — this is a pure additive geometric filter, doesn't touch the model, and is trivially disabled (`ENABLE_DUCK_SIZE_CHECK`) if it turns out to reject good sightings instead. Tolerance (`0.4x`-`2.5x`) deliberately wide/asymmetric to tolerate a partially-boxed real duck (head-only box, seen live) without false-rejecting it. Live-verified after a fresh boot: ratio held steady at 1.3-1.6x across ~40s of continuous detection, well inside tolerance, zero false rejections.

**Real incident, now fixed: two people sharing one physical Jetson.** The Jetson was found checked out on a teammate's feature branch (`OhlookitsAlex`, motor/PID work) with a stale local `main`, after that branch's testing session ended — cost real debugging time chasing a "why doesn't the dashboard show distance" symptom that was actually just wrong/old code running. **Fix (merged): `search_and_rescue.py` now prints `[git] running from branch 'X' @ commit (uncommitted local changes)` as the first line on startup** — check this before trusting any test result. Agreed team workflow going forward: Jetson defaults to `main` (pulled fresh) between sessions, `git checkout <branch> && git pull` before hardware testing, `git checkout main && git pull` when done, quick heads-up message to the other person when using the Jetson for your own branch.

**OPEN ISSUE, unresolved as of session end — resume here next session.** After fixing the branch mismatch above and confirming (via direct terminal log AND a raw `curl`/browser fetch of `/state`) that the backend produces a fully valid, fresh stereo reading continuously (`distance_m`, `t` within the last second, correct size-check ratio), **the dashboard's "distance to duck now" box still never renders it** — permanently shows "no current stereo lock" even during a 40+ second window of unbroken successful detections.

Ruled out so far, with evidence:
- **Not a stale reading / clock skew** — `/state`'s `latest_stereo.t` matches Jetson's own NTP-synced system clock; comparing directly against the viewing device's clock showed no dramatic offset either (though this wasn't tested with sub-second precision — a DevTools console one-liner `fetch('/state').then(r=>r.json()).then(s=>console.log(s.latest_stereo, Date.now()/1000 - s.latest_stereo.t))` was proposed to get an exact number, not yet run).
- **Not a JS exception silently breaking `poll()`** — read through the full function; if any line threw, the outer `.catch()` would show "connection lost, retrying..." in the nav status box instead of the normal position/heading text that's actually showing. All referenced DOM element IDs (including the newer `pidMaxHeadTrim`/`pidKlock`/`pidMaxLock` inputs from Alex's PID-heading-lock merge) were confirmed present in the HTML.
- **Not a hard-refresh/cache issue** — tried, no change.
- **Not related to the robot being stationary/not started** — vision/detection runs on its own thread, independent of `/odom` and the Start button; a `/state` fetch with `"started": false` still showed a fully valid, fresh `latest_stereo`.

**New lead, not yet chased down:** the terminal log shows `"Dashboard: right video client connected"` immediately followed by `"...disconnected"` about **330ms later** — the MJPEG camera feed connection is flapping (repeatedly reconnecting), which doesn't fit a stable session. Untested hypothesis: WiFi instability between the viewing device and the Jetson (or a Flask dev-server limitation under this connection pattern) rather than a logic bug in `search_and_rescue.py` itself. **Next step: reproduce with the viewing device on wired Ethernet instead of WiFi to isolate the network as the cause**, and/or run the DevTools one-liner above for an exact, unambiguous freshness-check number instead of inferring from indirect signals.

Also unresolved, lower priority: the micro-ROS agent was found down (dashboard showing "NO /odom") after an unrelated accidental `pkill -f micro_ros_agent` earlier the same session — unclear whether it was ever restarted; doesn't block the stereo-lock investigation (vision thread is independent of odom) but should be checked before trusting any `/odom`-dependent test.

**RESOLVED 2026-08-27 night.** Root cause was neither the network nor the backend — it was **Chrome throttling `setInterval` on a backgrounded browser tab**. Confirmed two ways: a manual DevTools-console `fetch('/state')` call (which bypasses `setInterval` entirely) always returned fresh data instantly, even at the exact moment the page's own polling loop was showing "no current stereo lock"; and a hard reload (which also re-focuses the tab, resetting Chrome's throttle) fixed the display immediately. The MJPEG-flapping and caching leads above were both directly disproven by measurement, not just abandoned — `/state` response time stayed ~25-60ms even under simulated concurrent video-stream load, and adding `Cache-Control: no-store` + `{cache:'no-store'}` (kept anyway, harmless) made no difference. **Practical takeaway: keep the dashboard tab focused/visible while monitoring live data.**

**New standalone `/vision` debug page** (same Flask app, `http://<jetson-hostname>.local:8080/vision`) — split out from the main dashboard specifically because that page had accreted a lot of PID-tuning UI (see Lessons Learned) unrelated to vision debugging. Shows just the two camera feeds, the distance readout(s), and a live diagnostic strip (`pollCount`/`tickCount` on independent 400ms/100ms timers) — that diagnostic strip is what made the throttling bug provable in the first place, worth reusing for any future "is this a data bug or a browser bug" question on this dashboard.

### Point-Cloud 3D Position Estimate + RViz2 Live Debugging (2026-08-27 night – 2026-08-28) — OPEN, resume here

**Second, independent distance/3D-position method, built alongside the existing centroid-disparity one.** Motivation: the centroid method structurally can't get past ~6-10% error (see the ruler benchmark above) since it's a single disparity value with no averaging to reject jitter — vịt's target is 1-2%. Pipeline: `cv2.StereoSGBM` dense stereo on the already-rectified frame pair → `cv2.reprojectImageTo3D` → crop the *output* point cloud to the duck's already-detected box (not the input — row-cropping before matching would silently invalidate the reprojection's principal point) → median X/Y/Z of valid points → transform through `camera_optical → base_link → world`, composing the rotation fresh at runtime (a small quaternion-multiply helper) rather than reusing `stereo_depth_argus.launch.py`'s fixed quaternion, since that one assumes an unyawed camera mount and this rig is yawed 45° for perimeter-hugging. Deliberately does not revive the Isaac ROS `stereo_image_proc` pipeline for this — it needs the full Docker container + rectified `visual_slam` launch, which claims the cameras exclusively and can't coexist with this script's own capture (same reasoning already given for the centroid method above).

**Real bug found and fixed on first live test: `cv2.reprojectImageTo3D` was returning consistently negative depth for real, in-front-of-camera geometry.** A diagnostic log (added specifically to chase this down) showed z values like `[-2.22, -0.44]m` at pixels that otherwise passed the disparity check — matching the centroid method's real ~0.78m distance in magnitude, just sign-flipped. Root cause: X/Y/Z in OpenCV's Q-matrix reprojection formula all share the same `(-Tx/disparity)` scale factor, so this calibration's baseline sign convention flipped all three axes together, uniformly — not a per-axis bug. Fix: negate the whole point cloud once, right after reprojection. Same *class* of issue as the already-documented `LEFT_SENSOR_ID`/`RIGHT_SENSOR_ID` swap bug elsewhere in this file (an unverified sign-convention assumption), different mechanism.

**Confirmed live after the fix, at multiple real distances: the two independent methods agree within 1-2cm** (0.49-0.51m vs 0.50m at ~0.5m; 0.75-0.76m vs 0.78-0.79m at ~0.78m), with `n_points` in the 8,000-24,000 range and compute time steady around ~1000-1050ms (matches a rate-limit estimate borrowed from Isaac's own `stereo_image_proc` measurement, not just a guess). **Cross-method agreement is not the same as confirmed accuracy** — a real ruler ground-truth benchmark (same protocol as the 0.5/1.0/1.5m test above) is the actual next step to answer "does this get us to 1-2%," and is still not done as of session end.

**RViz2 set up for the first time on this project**, running via SSH X11-forwarding from WSL2 (Windows 11's built-in WSLg) to the Jetson — avoids installing ROS2 on Windows/WSL2 at all; `rviz2` runs natively on the Jetson (already has the full ROS2 environment), only rendered pixels travel over X11 to the Windows desktop. Two real environment bugs hit getting there:
- **`~/.Xauthority` was a directory, owned by root, instead of a file** — caused `X11 connection rejected because of wrong authentication`. Fixed by `rmdir ~/.Xauthority` (the parent dir was user-writable, so no sudo needed) and letting SSH regenerate it cleanly on the next connection.
- **RViz2's Fixed Frame couldn't be set to `odom` at all** (`Frame [odom] does not exist`) because `search_and_rescue.py` never published any `/tf` — a message's `header.frame_id` (already `"odom"` on the Marker/Odometry messages) does **not** register a frame with tf2; only actual `/tf` publications do. Fixed by adding a real `odom → base_link` broadcast inline in `on_odom` (the same data `jetson/nav2/odom_to_tf.py` already publishes as a separate node, just inlined here so it works without that node running) plus a static `base_link → camera_optical` transform (rotation composed at runtime, accounting for the rig's 45° yaw — explicitly *not* reusing `stereo_depth_argus.launch.py`'s fixed quaternion, which assumes a straight mount).

**RViz2 additions for actually debugging accuracy, not just watching a dot move:** a `TEXT_VIEW_FACING` marker floats the live distance number next to the position marker (previously only readable from the terminal log); the duck-position sphere now cycles through 20 marker ids instead of always overwriting id `0`, so ~15-20 seconds of recent sightings persist simultaneously as a visible scatter — a tight cluster means stable/trustworthy, spread out means real per-reading noise, which a single overwritten number can't show; and the raw extracted point-cloud region is now published on `/duck_pointcloud_region` (`PointCloud2`, `camera_optical` frame) so the actual per-pixel stereo match data is inspectable, not just the reduced median. Intended debugging workflow: orbit the point cloud in RViz2 to check whether it's elongated along the camera's depth axis more than laterally (confirms the accuracy ceiling is the expected `1/disparity` physics, not a bug — steers the fix toward frame-averaging/sub-pixel refinement, not model changes), whether outlier clusters exist separate from the main mass (background bleeding into the detection box), and whether the cloud's shape is recognizably duck-like or just diffuse noise.

**Recurring operational issue, now scripted:** the micro-ROS agent gets stuck forever at `logger setup` whenever the ESP32's connection state is stale relative to a freshly (re)started agent — happens after every Jetson reboot, and sometimes after just restarting the agent alone. The previously-documented fix (`idf.py monitor` briefly) needs a real TTY and can't run over a plain SSH command; `esptool --port /dev/ttyUSB0 chip_id` triggers the same hardware reset (via the RTS pin) non-interactively, no TTY needed. Wrapped into one idempotent script: `jetson/tools/start_microros_agent.sh`.

**OPEN as of that session end (superseded by the 2026-08-29 section below):**
1. ~~Point cloud not visible in RViz2 despite the topic/TF being correctly wired.~~ **Root-caused and fixed 2026-08-29 — see below.**
2. ~~Real ruler ground-truth benchmark for the point-cloud method.~~ **Done 2026-08-29 — see below.**
3. Jetson lost network connectivity entirely at that session's end (both direct IP and mDNS unreachable, not just slow) — cause was never confirmed, resolved itself by the next session (simple reboot/idle timeout, not chased further since it didn't recur).

### Core-Region Promotion + TF-Stamp Root Cause + Calibration Investigation (2026-08-29)

**Ruler benchmark for the point-cloud method finally run, and it surfaced a real, fixable bug: edge-bleeding from a loosely-fit detection box was dragging the full-box median toward "farther."** Diagnosed via a diagnostic-only core/eroded-region computation (`POINTCLOUD_EROSION_MARGIN_FRAC=0.25`) logged alongside the production number — one live frame showed full-box=2.10m vs. the centroid method's simultaneous 0.87m, while the SAME frame's core region gave 0.90m. Promoted the core-region median to be the actual production `distance_m`/`x_cam`/`y_cam`/`z_cam` (falling back to the full-box reading when the eroded region doesn't clear `POINTCLOUD_MIN_VALID_POINTS`, so a small/distant duck doesn't just drop out). Re-ran the same 0.3/0.5/1.0m ruler protocol: **0.30m→0.28m (was 0.43m, 43% err), 0.50m→0.47m (was 0.50m, ~0% err but now closer still), 1.00m→0.89-0.91m stable within 2cm (was 0.92-1.29m swinging 37cm)** — at 1m specifically, the full-box number kept swinging as high as 2.13m on the same frames the core-region number held steady, live proof the fix addresses the diagnosed mechanism.

**Second, independent bug found while investigating: `/duck_pointcloud_region` never rendered in RViz2 despite correct topic/TF wiring, and it wasn't a display-config or discovery issue.** `ros2 topic echo /odom --field header.stamp` showed the ESP32/micro-ROS `Odometry` message's stamp is **always** `sec=0/nanosec=0`. `on_odom` was copying that zero stamp straight into the `odom → base_link` TF broadcast, pinning the transform at "time 0" in tf2's buffer forever — `x`/`y`/`theta` kept updating fine (same callback, different fields) and the sphere Markers kept rendering fine (placed directly in the `odom` frame via already-computed world coordinates, no tf2 lookup needed), which is why only the PointCloud2 display was silently broken. Fixed by stamping with the Jetson's own wall clock (`self.get_clock().now().to_msg()`) instead, same pattern already used for the static camera TF. Confirmed live via `tf2_echo`: "latest data is at time 0.000000" before the fix, real advancing timestamps and successful lookups after.

**Code review of this change caught two more real issues, both fixed before commit:** the `/duck_pointcloud_region` debug cloud was still unconditionally publishing the full-box mask even after core-region became the production source (would have shown misleading background points in the exact debug view meant to rule that out); and the full-box/core-region median math was duplicated (extracted a shared `_median_xyz_distance()` helper — the same "two copies, easy to fix one and forget the other" risk that caused the original edge-bleeding confusion).

**With the debug cloud fixed, the orbit-diagnostic (elongation check) was re-run for real and showed a tight, compact cluster — not elongated.** This argues against the `1/disparity` quantization-ceiling hypothesis (which would predict growing scatter/elongation at range) and for a systematic scale bias instead, which motivated a real investigation into the calibration file. **Four hypotheses tested with concrete evidence, all ruled out — calibration itself is confirmed NOT the source of the residual ~5-11% bias:**
1. Checkerboard square size — the calibration's own self-check (`baseline_m=0.0854m` vs. ~0.083m measured mount baseline) is only 2.9% off, and in the wrong direction to explain an under-read.
2. An earlier single-camera calibration vịt remembered — confirmed to predate the second camera entirely (the current file's `baseline_m`/`T` fields could only come from a real 2-camera `cv2.stereoCalibrate` run).
3. Pinhole vs. fisheye lens model (this camera is a 160° FOV IMX219 module) — ran `stereo_calibrate_fisheye.py` (built in an earlier session, never actually executed until now; fixed a real `float32`/`float64` dtype crash in `cv2.fisheye.stereoCalibrate` along the way) against the same `stereo_pairs/` batch. Fisheye baseline came out to 0.0856m — within 0.2% of the pinhole model's 0.0854m. Two independent distortion models converging this closely rules out a lens-model mismatch.
4. Live capture resolution vs. calibration capture resolution — both confirmed 1280×720.

**Residual ~5-11% bias is logged as open but not a quick-check item anymore** — the cheap/moderate hypotheses are exhausted; a real next step would need a proper controlled experiment (many repeated trials against a precisely-known target at several distances) rather than another single-check hypothesis. Current accuracy (~5-10% error, stable — a large improvement from the session's starting point) was kept as-is rather than chased further.

### Photometric L/R Normalization + Point-Cloud Range Instability Root Cause + Centroid Gate (2026-09-04)

**Session goal going in:** reduce the near-field point-cloud bias from ~5-10% to 4-5% (branch 1 of a two-branch plan, branch 2 being a longer-term NVIDIA-hardware-accelerated stereo pipeline, not started). What was actually found along the way turned out bigger than the original goal — full numbers kept here for report-writing.

**Calibration/hardware re-verified live on the Jetson (all pulled from the real `stereo_calibration.npz` and a live rectification run, not assumed):**
- `baseline_m = 0.08540m` vs. ~0.083m physically measured mount baseline — 2.9% off, and in the wrong direction to explain an under-read (confirms the 2026-08-29 finding, re-checked fresh).
- `image_size = [1280, 720]` in the calibration file, derived from the actual calibration images' own shape — confirmed to exactly match the live capture pipeline's `width=1280, height=720`.
- Rectified `fx = 875.515` (both `P1[0,0]` and `P2[0,0]` match exactly, as expected post-rectification), rectified ROI `(0,1,1280,719)` left / `(0,0,1280,720)` right — essentially the full frame, no meaningful crop/vignetting from rectification.
- Real distortion coefficients: `D1 = [-0.390, 0.225, 0.0053, -0.00077, -0.078]` (significant barrel distortion, as expected for this lens).

**L/R photometric (brightness) mismatch measured live:** a 6-second/38-sample capture (duck+camera static) showed each camera's own auto-exposure is very stable frame-to-frame (left std=0.28, right std=0.29 gray levels) but the two physical camera modules converge to different absolute brightness for the same scene — left mean=107.7, right mean=111.1, a consistent ~3.2/255 (~3%) gap, not flicker. **Fix (PR #67, merged):** rescale the right frame's mean onto the left's, every frame, right before `stereo_matcher.compute()` — cheaper and more self-adjusting than an Argus hardware exposure/gain lock (which would need a hand-picked fixed exposure, risking under/over-exposure or motion blur once the robot drives).

**L/R capture synchronization measured live:** no hardware trigger/genlock exists between the two cameras — each runs an independent free-running `nvarguscamerasrc` pipeline (`appsink drop=1`), read sequentially in Python. 20 back-to-back samples: the software gap between finishing the left `.read()` and getting the right frame back averaged 2.38ms (min 0.48ms, max 4.92ms) — tight, but this only measures call latency, not true frame-capture-instant alignment (no PTS/timestamp comparison was possible with this pipeline). Worst-case theoretical skew is bounded by one frame period (~16.7ms at the negotiated 60fps sensor mode). **Not the cause of anything found this session** (all tests were with a fully static duck+robot) — flagged as an open risk specifically for future driving-while-detecting accuracy, not chased further.

**A serious, previously-unknown bug found while live-testing the photometric fix: point-cloud distance becomes wildly unreliable beyond ~0.5m, in a way the near-field 5-10% bias work never surfaced.** Ruler benchmark, duck static at each distance, point-cloud (core-region) method:
- 0.3m real → stable 0.36m every tick (10/10 ticks, 4500-6000 core points each) — off by +20%, but *stable*.
- 0.5m real → RViz showed a range of 0.44-0.5m (already unstable, just not extreme).
- 1.0m real → RViz showed a range of 0.41-0.96m.
- 1.3m real → full log captured (see `COMMIT_HISTORY.md`/git history for the raw dump): **three discrete, repeating clusters** — ~0.37-0.44m (core `[USED]`, 100-650 points), ~1.53-1.56m (fell back to full-box), ~2.43-2.46m (fell back to full-box) — while the **centroid method stayed rock-stable at 1.30-1.32m the entire time (~1% error)**, every single tick, using the exact same detection box.

**Root cause, confirmed with an A/B test (reverted to the pre-photometric-fix baseline and reran the identical 1.3m test — produced the exact same 3-cluster pattern), so it predates today's normalize fix entirely:** at 1.3m, `core_disparity_ok` was **0 points on every fallback tick** while the full box always had 100+ valid points *on the same tick* — meaning the few pixels SGBM can match confidently sit right at the box edges (background), not the center (the duck itself). A fixed-size duck's box height shrinks with range (`≈ fx·duck_height/distance` — 379px @0.3m, 227px @0.5m, 114px @1.0m, 87px @1.3m; after the existing 25%-per-side erosion, the surviving "core" crop is ≈190/114/57/44px respectively). The duck's smooth, low-texture surface gives SGBM too little signal to match confidently at its own center at long range — a real block-matching-stereo limitation, not a tunable-parameter bug. 0.3m stays clean only because there's still an abundance of pixels+texture despite the same relative crop.

**Adaptive core-erosion sizing was tried live as the "fix the crop" hypothesis — tested and rejected, not merged.** Reduced the erosion margin for small/far boxes (10% instead of 25% below a 150px box-height threshold) so core wouldn't crop down to zero. Live retest at 1.3m: core never hit 0 points again (39-806 points every tick) — but the readings got *noisier*, not better: 0.20m, 0.37m, 0.39m, 1.09m, 1.20m, 1.52m, 1.55m all appeared across consecutive ticks for the same static 1.3m target. Confirms the mechanism is a texture problem, not a crop-size problem — a larger core just admits more of the same background-contaminated pixels.

**Fix shipped (PR #68): gate point-cloud's distance against the centroid method, which was accurate at every distance tested this session (~1% error at 1.3m).** If point-cloud's raw distance disagrees with centroid's by more than `POINTCLOUD_CENTROID_MAX_DISAGREEMENT_FRAC = 0.30` (30%), rescale point-cloud's position vector onto centroid's distance (keeps point-cloud's lateral direction, discards a depth that's likely measuring the wrong object) instead of trusting it outright. Visible as `[GATED, raw was X.XXm -> ...]` in the ROS log and a `[gated -> using centroid]` tag on the web dashboard's point-cloud box; `latest_pointcloud_reading['gated']` (bool) is in the `/state` JSON too. **This is a reliability patch (caps how wrong a single reading can be), not an accuracy fix — the original near-field 5-10% bias goal is still open, unrelated to this bug.** Not yet live-verified against the full 0.3-1.3m sweep as of session end — resume there.

**Also confirmed via code read: today's point-cloud bug never affected robot safety/navigation.** `latest_pointcloud_reading`/`latest_stereo_reading` are read only by the web dashboard's display code — the current mission (fixed waypoint perimeter patrol) doesn't use either for any navigation decision. Safe for a normal test-drive session throughout.

**Operational note:** running RViz2 (via WSL2 X11 forward) at the same time as an actual motor-driving test is technically non-conflicting (RViz2 only subscribes, never publishes to `/cmd_vel`) but not recommended on this Jetson — CPU is already the bottleneck (`Vision loop: 0.54Hz` observed live, target 5.0Hz, YOLO+SGBM-bound) and RViz2's own TF/rendering load competes for the same CPU the motor PID loop needs. Use the lighter web dashboard (`:8080`) during real drive tests; reserve RViz2 for stationary diagnostic sessions.

**Operational issue hit repeatedly this session, now understood: `nvargus-daemon` (the system service owning the CSI cameras) can get stuck in a bad internal state that survives killing the Python client.** After a `Ctrl+C` + relaunch, camera open sometimes still failed with `Failed to create CaptureSession` even with no other process holding the camera (confirmed via `pgrep`) — `journalctl`/`systemctl status nvargus-daemon` showed the daemon's own log spamming `(Argus) Error InvalidState: CaptureProvider destroyed` on every new client connection attempt, not just the failing one. Killing the Python process is not enough in this state — fix is `sudo systemctl restart nvargus-daemon`, then relaunch. Worth trying this FIRST (before more `pgrep`/kill cycles) if `Failed to create CaptureSession` recurs.

**End-of-session status (2026-09-04):** the point-cloud distance box (with the `[gated -> using centroid]` indicator) was ported from the separate `/vision` debug page onto the main dashboard (same page as the map/PID tuning/manual drive tester) so it's visible without switching pages — deployed, dashboard confirmed loading correctly. Centroid method's accuracy was re-confirmed live via the main dashboard's own smoothed readout across all four tested distances: 0.3m→0.34m, 0.5m→0.54-0.59m, 1.0m→1.1m, 1.3m→1.3m — consistent with the terminal-log-based ~1% figure at 1.3m from earlier in the session. **Not yet confirmed: the gate itself (PR #68) triggering correctly on the newly-added main-dashboard point-cloud box** — the readings above were read from the centroid box, not the point-cloud box, before the Jetson was handed off to a teammate for an unrelated test drive. First thing to check next session: watch the point-cloud box (not the centroid box) at 1.0m/1.3m and confirm the `[gated -> using centroid]` tag appears when it should. PR #68 (`vision/pointcloud-centroid-gate`) is open, deployed on the Jetson, not yet merged.
...
![](<../docs/images/PID pipeline.drawio.png>)
